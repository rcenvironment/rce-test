 ...
 * 
 */
public class LargeScaleScenarioTests extends AbstractLargeScaleTest {
    ...
    public void testConcurrentlyGrowingRandomNetwork() throws Exception {
        ...
        for (int i = 0; i < allInstances.length - 1; i++) {
            ...
            testTopology.connect(connectedInstanceIndex, i + 1, true);
        }
        waitForNextMessage();
        waitForNetworkSilence();
        assertTrue("Instances did not converge", instanceUtils.allInstancesConverged(allInstances));
    }
    ...
    public void testDuplexRingFailureRecovery() throws Exception {
        ...
        for (int i = 0; i < epochs; i++) {
            ...
            for (VirtualInstance vi : allInstances) {
                // FIXME check: this assertion fails frequently
                assertEquals(ERROR_MSG_NUMBER_OF_NODES, testSize, vi.getTopologyMap().getNodeCount());
                assertEquals(ERROR_MSG_NUMBER_OF_LINKS, testSize * 2, vi.getTopologyMap().getLinkCount());
            }
            ...
            NetworkResponse response = sender.performRoutedRequest(message, receiver.getNodeId(), DEFAULT_REQUEST_TIMEOUT);
    ...
    public void testDuplexChainFailureRecovery() throws Exception {
        ...
        for (int i = 0; i < epochs; i++) {
            ...
            try {
                ...
                NetworkResponse response = sender.performRoutedRequest(messageContent, receiver.getNodeId(), DEFAULT_REQUEST_TIMEOUT);
    ...
    public void testMessageConfirmation() throws Exception {
        ...
        for (int i = 0; i < testSize; i++) {
            ...
            NodeIdentifier receiverId = receiver.getConfigurationService().getLocalNodeId();
    ...
    public void testMessageFailure() throws Exception {
        ...
        // this should work
        NetworkResponse response = sender.performRoutedRequest(DUMMY_REQUEST_CONTENT, failingInstance.getNodeId(), DEFAULT_REQUEST_TIMEOUT);
        ...
        try {
            response = sender.performRoutedRequest(DUMMY_REQUEST_CONTENT, failingInstance.getNodeId(), DEFAULT_REQUEST_TIMEOUT);
    ...
    public void testDisconnectedNetworkParts() throws Exception {
        ...
        try {
            ...
            response =
                allInstances[0].performRoutedRequest(DUMMY_REQUEST_CONTENT, allInstances[allInstances.length - 1].getNodeId(),
