...
public class OutputWriterEndpointEditDialog extends EndpointEditDialog {
    ...
    protected Text createLabelAndTextfield(Composite container, String text, String dataType, String value) {
        ...
        final Composite existingFileHandlingComp = new Composite(container, SWT.NONE);
        existingFileHandlingComp.setLayout(new GridLayout(2, false));
        new Label(existingFileHandlingComp, SWT.NONE).setText("Handle \nexisting file: ");
        final Combo existingFileHandlingCombo = new Combo(existingFileHandlingComp, SWT.READ_ONLY);
        final String[] existingFileOptions =
        { Messages.handleAppend, Messages.handleAutoRename, Messages.handleDoNothing, Messages.handleOverride };
        existingFileHandlingCombo.setItems(existingFileOptions);
        existingFileHandlingCombo.select(0);
