...
public class OutputWriterComponent extends DefaultComponent {
    ...
    public void processInputs() throws ComponentException {
        ...
        if (!fileToWrite.exists()) {
            ...
            case "OVERRIDE":
                ...
                break;
            case "APPEND":
                if (input.getDataType().equals(DataType.FileReference)) {
                    try {
                ...
                } else if (input.getDataType().equals(DataType.Boolean) || input.getDataType().equals(DataType.Float)
                    || input.getDataType().equals(DataType.Integer) || input.getDataType().equals(DataType.ShortText)) {
                    try {
                        /*
                         * FileWriter writer = new FileWriter(fileToWrite, true); writer.write(inputName + input.toString() + "\n");
                         * writer.close();
                         */
                        FileUtils.writeStringToFile(fileToWrite, inputName + "\t" + input.toString() + "\r\n", true);
                    } catch (IOException e) {
                        throw new ComponentException("Failed to write output: " + fileToWrite.getAbsolutePath(), e);
                    }
                }
                break;
            case "AUTORENAME":
    ...
    public void writeFile(TypedDatum input, String path) throws ComponentException {
        ...
        default:
            ...
            incFile = new File(path);
            try {
                FileUtils.writeStringToFile(incFile, input.toString(), false);
            } catch (IOException e) {
                throw new ComponentException("Failed to write output: " + incFile.getAbsolutePath(), e);
            }
            break;
        }
