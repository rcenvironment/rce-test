...
import de.rcenvironment.rce.authentication.CertificateUser;
import de.rcenvironment.rce.authentication.LDAPUser;
import de.rcenvironment.rce.authentication.SingleUser;
...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    /**
     * 
     * Constant that holds the name of the environment property for specifying how 
     * referrals encountered by the service provider are to be processed. 
    ...
    /**
     * 
     * Tries to set up and bind the LDAP-Connection and sets dirContext to an initialized directory service.
     * Reads properties from the file ldap.properties and sets the context.
     * 
    ...
    private List<X509Certificate> getCertificateAuthorities() {
        ...
        for (String path : myConfiguration.getCaFiles()) {
            try {
                absPath = configurationService.getAbsolutePath(bundleSymbolicName, path);
