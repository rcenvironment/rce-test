...
public class DoubleClickDialog extends Dialog {
    ...
    private void fillTableWithContent() {       
        de.rcenvironment.rce.component.datatype.Table vals = cval.getValues();
        
        //Create columns
        for (int loopIndex = 0; loopIndex < vals.getMaximumColumnIndex() + 1; loopIndex++) {
            TableColumn column = new TableColumn(table, SWT.NULL);
            column.setResizable(true);
        }
        
        //Set table content
        for (int row = 0; row < vals.getMaximumRowIndex() + 1; row++) {
            TableItem item = new TableItem(table, SWT.NULL);
            for (int col = 0; col < vals.getMaximumColumnIndex() + 1; col++) {
                item.setText(col, vals.getCell(row, col).getValueAsString());
            }
        }
        for (int loopIndex = 0; loopIndex < vals.getMaximumColumnIndex() + 1; loopIndex++) {
