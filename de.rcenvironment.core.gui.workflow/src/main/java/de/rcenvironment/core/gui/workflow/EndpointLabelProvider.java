...
public class EndpointLabelProvider extends LabelProvider {
    ...
    private static final int INPUT_REQUIRED_DECORATOR_LOCATION = 2; // 2 = bottom left
    ...
    private Map<Image, Image> connectedOnlydecoratorCache = new HashMap<>();

    private Map<Image, Image> requiredOnlydecoratorCache = new HashMap<>();

    private Map<Image, Image> connectedAndRequireddecoratorCache = new HashMap<>();
    ...
    private Image checkForDecorators(EndpointDescription endpointDescription, Image image) {
        ...
        if (inputConnected && inputRequired) {
            if (connectedAndRequireddecoratorCache.keySet().contains(image)) {
                image = connectedAndRequireddecoratorCache.get(image);
            } else {
                ...
                connectedAndRequireddecoratorCache.put(originalImage, image);
            }
            
        // Only Connected
        } else if (inputConnected && !inputRequired) {
            if (connectedOnlydecoratorCache.keySet().contains(image)) {
                image = connectedOnlydecoratorCache.get(image);
            } else {
                ImageDescriptor[] decorations = new ImageDescriptor[5];
                decorations[INPUT_CONNECTED_DECORATOR_LOCATION] = inputConnectedDecorationIcon;
                Image originalImage = image;
                image = createDecoratedImage(originalImage, decorations);
                connectedOnlydecoratorCache.put(originalImage, image);
            }
            
        // Only Required
        } else if (!inputConnected && inputRequired) {
            if (requiredOnlydecoratorCache.keySet().contains(image)) {
                image = requiredOnlydecoratorCache.get(image);
            } else {
                ImageDescriptor[] decorations = new ImageDescriptor[5];
                decorations[INPUT_REQUIRED_DECORATOR_LOCATION] = inputRequiredDecorationIcon;
                Image originalImage = image;
                image = createDecoratedImage(originalImage, decorations);
                requiredOnlydecoratorCache.put(originalImage, image);
            }
        }
        return image;
    }
    ...
    public void dispose() {
        ...
        for (Image image : connectedOnlydecoratorCache.values()) {
            image.dispose();
        }
        connectedOnlydecoratorCache.clear();

        for (Image image : requiredOnlydecoratorCache.values()) {
            image.dispose();
        }
        requiredOnlydecoratorCache.clear();

        for (Image image : connectedAndRequireddecoratorCache.values()) {
            image.dispose();
        }
        connectedAndRequireddecoratorCache.clear();
    }
}
