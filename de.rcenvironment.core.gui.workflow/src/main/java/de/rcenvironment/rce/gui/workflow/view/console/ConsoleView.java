 ...
 */
public class ConsoleView extends ViewPart implements NetworkTopologyChangeListener {
    ...
    private int lastKnownSequenceId = ConsoleModel.INITIAL_SEQUENCE_ID;

    private final Timer modelQueryTimer = new Timer();

    private final ConsoleModel consoleModel = ConsoleModel.getInstance();
    ...
    /**
     * A timer task that is used to periodically check the {@link ConsoleModel} for modifications.
     * This approach was chosen over a callback/observer structure to realize rate limiting of GUI
     * updates to ensure responsiveness in high CPU load situations.
     * 
     * @author Robert Mischke
     */
    ...
    /**
     * A Runnable that applies a given {@link ConsoleModelSnapshot} to the GUI. Intended to run from
     * the SWT event dispatch thread.
     * 
     * @author Robert Mischke
     */
