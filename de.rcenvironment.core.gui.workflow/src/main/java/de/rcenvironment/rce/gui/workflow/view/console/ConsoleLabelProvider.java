...
public class ConsoleLabelProvider extends LabelProvider implements ITableLabelProvider {
    ...
    // TODO better image for "meta info"?
    private Image metaInfoImage = ImageDescriptor.createFromURL(
        ConsoleView.class.getResource("/resources/icons/console_16.png")).createImage(); //$NON-NLS-1$
        ...
        if (element instanceof ConsoleRow) {
            ...
            if (0 == columnIndex) {
                ...
                if (ConsoleRow.Type.STDERR == consoleRow.getType()) {
                    ...
                    result = stdOutImage;
                } else if (ConsoleRow.Type.META_INFO == consoleRow.getType()) {
    ...
    public String getColumnText(Object element, int columnIndex) {
        ...
        if (element instanceof ConsoleRow) {
            ...
            // date and time
            case 1:
                returnValue = timestampDateFormat.format(consoleRow.getTimestamp());
                break;
            // text
            case 2:
                returnValue = consoleRow.getText();
                break;
            // component
            case 3:
                returnValue = consoleRow.getComponent();
                break;
            // workflow
            case 4:
                returnValue = String.valueOf(consoleRow.getWorkflow());
                break;
            default:
                // shouldn't happen
                break;
