...
import de.rcenvironment.commons.textstream.receivers.AbstractTextOutputReceiver;
import de.rcenvironment.commons.textstream.receivers.LoggingTextOutReceiver;
...
public abstract class WrapperBase<C, R> {
    ...
    private void uploadFileFromReference(String filename, String reference, CommandLineExecutor executor) throws IOException {
        ...
        File tempFile = TempFileUtils.getDefaultInstance().createTempFileFromPattern("upload." + filename + "-*.tmp");
        tempFile.deleteOnExit(); // safeguard
        try {
            getDataManagementService().copyReferenceToLocalFile(reference, tempFile);
            // monitoringListener.appendUserInformation("Uploading " + filename);
            executor.uploadToWorkdir(tempFile, filename);
        } finally {
    ...
    protected String downloadFileToReference(String filename, CommandLineExecutor executor) throws IOException {
        ...
        File targetFile = TempFileUtils.getDefaultInstance().createTempFileFromPattern("download." + filename + "-*.tmp");
        targetFile.deleteOnExit(); // safeguard
        try {
            executor.downloadFromWorkdir(filename, targetFile);
