...
public class WfCommandPlugin implements CommandPlugin {
    ...
    private static final String WRONG_STATE_ERROR = "Operation not possible in current workflow state";

    private static final String WORKFLOW_ID = "<workflow execution ID> ";
        ...
        if (wExecInf != null) {
            ...
            try {
                if (workflowExecutionService.getWorkflowState(executionId, nodeId).equals(WorkflowState.RUNNING)) {
                    ...
                    outputReceiver.addOutput(WRONG_STATE_ERROR);
                }
            } catch (CommunicationException e) {
                log.error("Exception while pausing workflow with executionId " + executionId);
            }
        }
    }
    ...
    private void performWfResume(final CommandContext context) throws CommandException {
        ...
        if (wExecInf != null) {
            ...
            try {
                ...
                log.error("Exception while resuming workflow with executionId " + executionId);
            }
        }
    }
    ...
    private void performWfCancel(final CommandContext context) throws CommandException {
        ...
        if (wExecInf != null) {
            ...
            try {
                ...
                log.error("Exception while cancelling workflow with executionId " + executionId);
            }
        }
    }
    ...
    private void performWfDispose(final CommandContext context) throws CommandException {
        ...
        if (wExecInf != null) {
            ...
            try {
                ...
                log.error("Exception while disposing workflow with executionId " + executionId);
            }
        }
    }

    private void performWfShowList(final CommandContext context) throws CommandException {
        
        TextOutputReceiver outputReceiver = context.getOutputReceiver();
        outputReceiver.addOutput("Workflow List");
    ...
    private WorkflowExecutionInformation getWorkflowForExecutionId(String executionId, TextOutputReceiver outputReceiver) {
        ...
        if (wExecInf == null) {
            outputReceiver.addOutput("Workflow with executionId " + executionId + " not found");
        }
        return wExecInf;
    }
