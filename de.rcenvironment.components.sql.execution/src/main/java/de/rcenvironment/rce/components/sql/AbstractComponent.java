...
public abstract class AbstractComponent extends DefaultComponent {
    ...
    private ComponentInstanceInformation instInformation;
    ...
    protected ComponentInstanceInformation getInstanceInformation() {
        return instInformation;
    }
    ...
    protected <T extends Serializable> T getVariableValue(final String variableName, final Class<T> clazz) {
    ...
    @Override
    public void onPrepare(final ComponentInstanceInformation instanceInformation) throws ComponentException {
        super.onPrepare(instanceInformation);
        this.instInformation = instanceInformation;
        this.inputConsumptionStrategy.setInputDefinitions(instanceInformation.getInputDefinitions());
        ...
        try {
             ...
             */
            final String dash = "-";
            defaultContext.set(CONTEXT_STATIC, "instanceInformation", instInformation);
            defaultContext.set(CONTEXT_STATIC, "componentId", instInformation.getIdentifier());
            defaultContext.set(CONTEXT_STATIC, "componentIdSafe", instInformation.getIdentifier().replace(dash, ""));
            defaultContext.set(CONTEXT_STATIC, "componentName", instInformation.getName());
            defaultContext.set(CONTEXT_STATIC, "workflowId", instInformation.getComponentContextIdentifier());
            defaultContext.set(CONTEXT_STATIC, "workflowIdSafe", instInformation.getComponentContextIdentifier().replace(dash, ""));
            defaultContext.set(CONTEXT_STATIC, "workflowName", instInformation.getComponentContextName());
            /*
             * Set the Outputs to the output context and prevent modifications through locking it for
             * write-access.
             */
            for (final String output : instanceInformation.getOutputDefinitions().keySet()) {
                defaultContext.set(CONTEXT_OUTPUT, output, instanceInformation.getOutput(output));
            }
             ...
             */
            for (final String key : instanceInformation.getConfigurationDefinitions().keySet()) {
                defaultContext.set(CONTEXT_CONFIG, key, instanceInformation.getConfigurationValue(key));
            }
        ...
        try {
            ...
            final SafeExecutor executor = new SafeExecutor() {
                ...
                protected Boolean execute() throws ComponentException {
                    return runInitialInComponent(inputsConnected);
                }
            };
            return executor.executeSafely();
        } finally {
            endRun();
        }
    }
    ...
    public boolean canRunAfterNewInput(Input newInput, Map<String, Deque<Input>> inputValues) throws ComponentException {
        boolean can = super.canRunAfterNewInput(newInput, inputValues);
        inputConsumptionStrategy.addInput(newInput);
        return inputConsumptionStrategy.canRun() && can;
    }

    @Override
    public final boolean runStep(Input newInput, Map<String, Deque<Input>> inputValues) throws ComponentException {
        try {
            startRun();
            ...
            for (Entry<String, Deque<Input>> inputName: inputValues.entrySet()) {
                inputValues.get(inputName.getKey()).removeFirst();
            }
            return executor.executeSafely();
        } finally {
            endRun();
        ...
        for (final String inputName : inputValues.keySet()) {
            ...
            if (inputs.size() > 1) {
                ...
                for (int index = 0; index < inputs.size(); ++index) {
                    ((List<Serializable>) value).add(inputs.get(index).getValue());
                }
            } else {
    ...
    protected Map<String, Class<? extends Serializable>> getOutputs() {
        return Collections.unmodifiableMap(instInformation.getOutputDefinitions());
    }

    protected Output getOutput(final String outputName) {
        return instInformation.getOutput(outputName);
    }
    ...
    protected boolean hasProperty(final String key) {
        return instInformation.getConfigurationDefinitions().keySet().contains(key); 
    }
    ...
    public String toString() {
        ...
        builder.append(": ");
        builder.append(getInstanceInformation().getName());
