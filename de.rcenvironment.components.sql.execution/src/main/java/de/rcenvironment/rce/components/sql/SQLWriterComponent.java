...
public class SQLWriterComponent extends AbstractSQLComponent {
    ...
    protected void runSqlBlockMode(final String sql) {
        ...
        try {
            ...
            try {
                ...
                // get the input
                final Map<String, VariantArray> input = extractFullDataInputs();
                // iterate over the rows
                for (final String inputName : input.keySet()) {
                    statement.clearParameters();
                    final VariantArray data = input.get(inputName);
                    final int[] dimensions = data.getDimensions();
                    final int rowCount = dimensions[0];
                    for (int row = 0; row < rowCount; ++row) {
                        for (int columnIndex = 0; columnIndex < dataIndexs.length; ++columnIndex) {
                            final TypedValue value = data.getValue(row, dataIndexs[columnIndex]);
                            statement.setObject(columnIndex + 1, value.getValue());
                        }
                        statement.execute();
                    }
                }
    ...
    protected Map<String, VariantArray> extractFullDataInputs() {
        final Map<String, VariantArray> result = new HashMap<String, VariantArray>();
        for (final Map.Entry<String, Class<? extends VariantArray>> entry : getFullDataInputs().entrySet()) {
            final String inputName = entry.getKey();
            final VariantArray inputData = getVariableValue(inputName, VariantArray.class);
    ...
    private boolean isCreateTableEnabled() {
        ...
        final Boolean createTableValue = getProperty(key, Boolean.class);
    ...
    private String createCreateTableStatement() {
        ...
        builder.append(getProperty(SqlComponentConstants.METADATA_TABLE_NAME_PROPERTY, String.class));
        builder.append("(");
    ...
    private InputMapping getInputMapping() {
        ...
        final String value = getProperty(key, String.class);
    ...
    private InputMode getInputMode() {
        if (inputMode == null) {
            ...
            inputMode = getProperty(key, InputMode.class);
        }
        return inputMode;
    }
