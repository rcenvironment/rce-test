...
import de.rcenvironment.rce.authentication.User.Type;
import de.rcenvironment.rce.configuration.ConfigurationService;
import de.rcenvironment.rce.login.internal.Messages;
import de.rcenvironment.rce.login.internal.ServiceHandler;
...
public abstract class AbstractLogin {
    ...
    private static final String LOGIN_FAILED_1_6 =
        "Authentication failed cause certificate is revoked by its certificate authority (CA)."; //$NON-NLS-1$
    ...
    public AbstractLogin() {
        ...
        loginConfiguration.setCertificateFile(configurationService.getAbsolutePath(ServiceHandler.getBundleSymbolicName(),
            loginConfiguration.getCertificateFile()));
        loginConfiguration.setKeyFile(configurationService.getAbsolutePath(ServiceHandler.getBundleSymbolicName(),
        ...
        try {
            ...
            while (!loginSuccess) {
                ...
                if (loginInput.getType() == Type.single) {                
                    notificationService.send(LOGIN_NOTIFICATION_ID, 
    ...
    /**
     * Must be implemented by login provider. Return the chosen login input. Can make use of
     * createLoginInput.
     * 
